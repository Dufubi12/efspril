...
- Can invoke `/business-case` for complete business case documents
- Can invoke `/visualize` to generate an HTML/PDF Investment Teaser from an existing analysis document.
- Provides quick analysis when commands not needed

## Visual Output Generation
To generate a "Visual Teaser" (One-Pager/Pitch Deck style) from a markdown analysis:
1. Ensure the markdown file has standard headers ("Startup Analysis:", "TAM/SAM/SOM", etc).
2. Run: `python .agent/skills/startup-analyst/scripts/generate_teaser.py <path_to_markdown>`
3. Output: `teaser.html` and `teaser.pdf` in the same directory.
